...
//            System.out.println("Data: " + list.peekString());
    ...
    public static void pump(final InputStream is, final DataSink ds, final CompletedCallback callback, UploadProgressHandler uploadProgressHandler) {
        pump(is, Integer.MAX_VALUE, ds, callback,uploadProgressHandler);
    }

    public static void pump(final InputStream is, final long max, final DataSink ds, final CompletedCallback callback,UploadProgressHandler uploadProgressHandler) {
                    ...
                    do {
                        if (!pending.hasRemaining()) {
                            ...
                            if(uploadProgressHandler != null){
                                uploadProgressHandler.progress(totalRead);
                            }
                            b.position(0);
                            b.limit(read);
                            pending.add(b);
    ...
    public static void pump(final File file, final DataSink ds, final CompletedCallback callback,UploadProgressHandler uploadProgressHandler) {
        try {
